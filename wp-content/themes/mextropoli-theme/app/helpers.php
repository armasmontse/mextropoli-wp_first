 ...
 *
 */
function getInitialValue($list, $slugProp, $slugName, $returnProp)
{
    $currentTypeId = -1;

    if(array_key_exists($slugName, $_GET)){
        $list = array_filter($list, function($item) use ($slugProp, $slugName){
            return $item->$slugProp == $_GET[$slugName];
        });

        if(count($list)){
            $currentTypeId = array_shift($list)->$returnProp;
        }
    }
